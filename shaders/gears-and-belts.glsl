...
float innerGear(vec2 p, float dir){
    p*=Rot(radians(-SLOWTIME*45.+45.)*dir);
    vec2 prevP = p;

    //p*=Rot(radians(SLOWTIME*45.+20.));
    p = DF(p,7.);
    p-=vec2(0.24);
    p*=Rot(deg45);
...
vec3 gear(vec2 p, vec3 col, float dir){
    ...
    p*=Rot(radians(SLOWTIME*45.+13.)*-dir);
    p = DF(p,7.);
    p-=vec2(0.23);
    p*=Rot(deg45);
    ...
    p*=Rot(radians(SLOWTIME*30.-30.)*dir);
    p = DF(p,6.);
    p-=vec2(0.14);
    p*=Rot(radians(45.));
    ...
    p*=Rot(radians(SLOWTIME*25.+30.)*-dir);
...
vec3 item3(vec2 p, vec3 col, float dir){
    ...
    p*=Rot(radians(sin(SLOWTIME*dir)*120.));
    vec2 prevP = p;
